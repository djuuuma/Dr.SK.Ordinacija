...
Ukoliko želite preuzeti, pokrenuti i testirati aplikaciju na Vašem računaru:

1. **Klonirajte ili preuzmite folder sa kodom.**
2. **Instalacija potrebnih paketa:**
   Otvorite terminal unutar foldera projekta i pokrenite:
   ```bash
   npm install
   ```
3. **Pokretanje razvojnog (Dev) servera:**
   Poslije isntalacija kucajte:
   ```bash
   npm run dev
   ```
   Aplikacija će se pokrenuti lokalno, najčešće na linku: [http://localhost:5173/](http://localhost:5173/)
